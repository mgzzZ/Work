...
- (void)setup
{
    ...
    bgViewBottomC = [NSLayoutConstraint constraintWithItem:self.textViewBgView
                                                 ...
                                                 attribute:NSLayoutAttributeBottom
                                                multiplier:1.f
                                                  constant:ScreenHeight - defaultRect.origin.y - defaultRect.size.height];
...
-(void)keyboardWillHide:(NSNotification *)note{
    NSNumber *duration = [note.userInfo objectForKey:UIKeyboardAnimationDurationUserInfoKey];
    bgViewBottomC.constant = ScreenHeight - defaultRect.origin.y - defaultRect.size.height;
    [UIView animateWithDuration:duration.floatValue animations:^{
        [self setFrame:defaultRect];
        [self layoutIfNeeded];
        self.coverView.hidden = YES;
    }];
}
