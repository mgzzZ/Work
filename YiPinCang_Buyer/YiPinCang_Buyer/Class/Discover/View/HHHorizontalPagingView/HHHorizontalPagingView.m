...
- (void)updateFramepagingViewWithHeaderView:(UIView *)headerView headerHeight:(CGFloat)headerHeight{
    ...
    [self.headerView removeConstraint:self.headerOriginYConstraint];
    ...
    [self.currentScrollView  setContentInset:UIEdgeInsetsMake(self.headerViewHeight+self.segmentBarHeight, 0., self.currentScrollView.contentInset.bottom, 0.)];

    self.currentScrollView.contentOffset = CGPointMake(0., -self.headerViewHeight-self.segmentBarHeight);
...
- (void)configureSegmentView {
    ...
    if(self.segmentView) {
        ...
        [self.segmentView addConstraint:[NSLayoutConstraint constraintWithItem:self.segmentView attribute:NSLayoutAttributeHeight relatedBy:NSLayoutRelationEqual toItem:nil attribute:0 multiplier:1 constant:self.segmentBarHeight + 40]];
        //加40 是因为下面tab与上面有40 的间隙
    }
}
...
- (UIView *)segmentView {
    if(!_segmentView) {
        ...
        [_segmentView addSubview:whiteView];
        whiteView.sd_layout
        .leftSpaceToView(_segmentView,0)
        .rightSpaceToView(_segmentView,0)
        .topSpaceToView(_segmentView,0)
        .bottomSpaceToView(_segmentView,0);
...
- (void)configureSegmentButtonLayout {
    if([self.segmentButtons count] > 0) {
        ...
        if(CGSizeEqualToSize(self.segmentButtonSize, CGSizeZero)) {
            ...
            buttonHeight = self.segmentBarHeight + 40 - 10;
            //加40 是因为下面tab与上面有40 的间隙
        }else {
            buttonWidth = [[UIScreen mainScreen] bounds].size.width/(CGFloat)[self.segmentButtons count];
            buttonHeight = self.segmentButtonSize.height-10;
            buttonTop = (self.segmentBarHeight + 40 - buttonHeight)/2.f;
            //加40 是因为下面tab与上面有40 的间隙
            buttonLeft = 0;
        }
        ...
        .leftSpaceToView(_segmentView,0)
        .rightSpaceToView(_segmentView,0)
        .bottomSpaceToView(_segmentView,10)
        .heightIs(0.5);
        ...
        for(int i = 0; i < [self.segmentButtons count]; i++) {
            ...
            redView.tag = kPagingButtonTag + i + 100;
            [_segmentView addSubview:redView];
            if(i == 0) {
                [segmentButton setSelected:YES];
                segmentButton.titleLabel.textAlignment = NSTextAlignmentCenter;
                segmentButton.frame=CGRectMake(0, buttonTop, buttonWidth, buttonHeight-1);
                redView.frame = CGRectMake(0, buttonHeight -1, buttonWidth, 1);
                redView.hidden = NO;
            }else{
                [segmentButton setSelected:NO];
                segmentButton.titleLabel.textAlignment = NSTextAlignmentCenter;
                segmentButton.frame=CGRectMake(buttonWidth * i, buttonTop, buttonWidth, buttonHeight-1);
                redView.frame = CGRectMake(buttonWidth * i, buttonHeight -1, buttonWidth, 1);
                redView.hidden = YES;
            }
...
- (void)segmentButtonEvent:(UIButton *)segmentButton {
    if (segmentButton.tag != kPagingButtonTag + self.segmentButtons.count - 1) {
        ...
        [segmentButton setSelected:YES];
        UIView  *redView = [_segmentView viewWithTag:segmentButton.tag + 100];
        redView.hidden = NO;
    }
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    [cell.contentView addConstraint:[NSLayoutConstraint constraintWithItem:v attribute:NSLayoutAttributeBottom relatedBy:NSLayoutRelationEqual toItem:cell.contentView attribute:NSLayoutAttributeBottom multiplier:1 constant:scrollViewHeight == 0 ? -10 : -(cell.contentView.frame.size.height-v.frame.size.height)]];
