...
- (instancetype)initWithFrame:(CGRect )frame segmentStyle:(ZJSegmentStyle *)segmentStyle titles:(NSArray *)titles titleDidClick:(TitleBtnOnClickBlock)titleDidClick {
    if (self = [super initWithFrame:frame]) {
        ...
        self.backgroundColor = segmentStyle.segNavColor;
...
- (void)setupTitles {
    ...
    for (NSString *title in self.titles) {
        ...
        label.backgroundColor = self.segmentStyle.bagTitleColor;
        if (self.segmentStyle.isHaveBorder) {
            label.layer.borderWidth = 2;
            label.layer.borderColor = self.segmentStyle.bagTitleColor.CGColor;
        }
...
- (void)setUpLabelsPosition {
    ...
    CGFloat titleNewW = 0.0;
    CGFloat titleNewH = 0.0;
    if (!self.segmentStyle.isScrollTitle) {// 标题不能滚动, 平分宽度
        if (self.segmentStyle.isHaveBorder) {
            titleW = _currentWidth / self.titles.count;
            _currentWidth = _currentWidth - 5 * self.segmentStyle.marginW;
             titleNewW = _currentWidth / self.titles.count;
            titleNewH = titleH - self.segmentStyle.marginH;
        }else{
             titleW = _currentWidth / self.titles.count;
            titleNewW = titleW;
            titleNewH = titleH;
        ...
        for (ZJCustomLabel *label in self.titleLabels) {
            ...
            titleX = index * titleW;
            if (self.segmentStyle.isHaveBorder) {
                titleX = index * (titleNewW + self.segmentStyle.marginW);
                 label.frame = CGRectMake(self.segmentStyle.marginW +titleX, titleY + (self.segmentStyle.marginH / 2), titleNewW, titleNewH);
            }else{
                 label.frame = CGRectMake(titleX, titleY, titleNewW, titleNewH);
            }
            index++;
...
- (void)adjustUIWithProgress:(CGFloat)progress oldIndex:(NSInteger)oldIndex currentIndex:(NSInteger)currentIndex {
    ...
    //是否有边框
    if (self.segmentStyle.isHaveBorder) {
//        oldLabel.layer.borderColor = [UIColor lightGrayColor].CGColor;
//        currentLabel.layer.borderColor = [UIColor whiteColor].CGColor;
